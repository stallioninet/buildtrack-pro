...
const ICONS = {
  ...
  'git-branch': (
    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 3v12m0 0a3 3 0 103 3H15a3 3 0 100-6H9m-3 3V3m12 0v6" />
    </svg>
  ),
  'file-text': (
    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <aside className="w-64 bg-slate-900 text-white flex flex-col min-h-screen">
      ...
      <div className="px-6 py-5 border-b border-slate-700">
        <h1 className="text-xl font-bold tracking-tight">
          <span className="text-blue-400">Build</span>Track Pro
        </h1>
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        {navItems.map((item) => (
          ...
          >
            {ICONS[item.icon] || <span className="w-5 h-5" />}
            {item.label}
          </NavLink>
        ))}
      </nav>
      ...
      <div className="px-4 py-4 border-t border-slate-700">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-slate-400 truncate">{user?.roleDisplayName}</p>
          </div>
        </div>
      </div>
    </aside>
